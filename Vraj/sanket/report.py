...
def generate_reports(
    ...
    staff_states = store.get_staff_states(session_id)
    staff_events = store.get_staff_events(session_id)
    staff_visits = store.get_staff_visits(session_id)
...
def _build_html_report(
    ...
    staff_states: Optional[List[Dict[str, Any]]] = None,
    staff_events: Optional[List[Dict[str, Any]]] = None,
    staff_visits: Optional[List[Dict[str, Any]]] = None,
) -> str:
    """Renders self-contained control-room styled HTML report."""
    dur_min = (session["duration_s"] or 0.0) / 60.0

    staff_states = staff_states or []
    staff_events = staff_events or []
    staff_visits = staff_visits or []

    # Build staff section HTML
    if not staff_states:
        staff_section_html = "<p class='text-muted'>No roving invigilation staff active in this session.</p>"
    else:
        staff_table_rows = []
        for st in staff_states:
            st_id = st["staff_id"]
            st_status = st["status"]
            st_badge = "badge-alert" if st_status == "attention_elevated" else ("badge-accum" if st_status == "review_suggested" else "badge-calm")
            
            # Events for this staff member
            st_evts = [e for e in staff_events if e["staff_id"] == st_id]
            st_evts_html = ""
            if st_evts:
                st_items = []
                for ev in st_evts:
                    ...
                    <li class="{ev_cls}">
                        ...
                        <span class="rule-tag">{html.escape(ev['rule'])}</span>: {html.escape(ev['reason'])}
                    </li>
                    """)
                st_evts_html = f"<details class='event-details'><summary>{len(st_evts)} Supervision Observations</summary><ul>{''.join(st_items)}</ul></details>"
            else:
                st_evts_html = "<span class='text-muted'>Standard invigilation pattern; no unusual dwell or repeat visit observations</span>"

            staff_table_rows.append(f"""
            <tr>
                <td class="mono font-bold">{html.escape(st_id)} (Track {st['track_id']})</td>
                <td><span class="badge {st_badge}">{html.escape(st_status.replace('_', ' ').upper())}</span></td>
                <td class="mono font-bold">{st['score']:.1f}</td>
                <td class="mono">{st['median_dwell_s']:.1f}s</td>
                <td class="mono">{st['total_visits']}</td>
                <td class="mono">{st['total_dwell_s']:.1f}s</td>
                <td class="text-sm">{html.escape(st.get('last_reason') or 'Nominal')}</td>
            </tr>
            <tr>
                <td colspan="7" class="event-cell">{st_evts_html}</td>
            </tr>
            """)

        staff_section_html = f"""
        <table>
            <thead>
                <tr>
                    <th>Staff Member</th>
                    ...
                    <th>Median Dwell</th>
                    <th>Total Desk Visits</th>
                    <th>Cumulative Dwell</th>
                    <th>Last Observation Summary</th>
                </tr>
            </thead>
            <tbody>
                {''.join(staff_table_rows)}
            </tbody>
        </table>
        """
...
<body>
    <div class="container">
        ...
        <div class="section-card">
            <div class="section-title">Staff Invigilation Supervision & Attention Distribution</div>
            {staff_section_html}
        </div>

        <div class="footer">
            <p><strong>Notice to Invigilators & Center Supervisors:</strong> Candidate alerts and staff observations are automated behavioral analytics for human review and do not constitute determinations or allegations of misconduct.</p>
